...
func _ready() -> void:
	...
	place_border()
	dfs(starting_coords)
	add_sepcial_tile(Vector2i(1,0), 3)
...
#fuckcja znajduje pole bez ścian lub płytek specjalnych
func find_empty_tile():
	var pos = Vector2i(rng.randi_range(0,x_dim),rng.randi_range(0,y_dim))
	while(not is_walkable(pos)):
		pos = Vector2i(rng.randi_range(0,x_dim),rng.randi_range(0,y_dim))
	#print(pos)
	#print("find_empty_tile")
	return pos

#funkcja przyjume adres płytki w atlasie i liczbę płytek to generowania
func add_sepcial_tile(tile_adress: Vector2i, number: int):
	var this_tile
	for i in number:
		this_tile = find_empty_tile()
		set_cell(0,this_tile , 0, tile_adress)
	pass
